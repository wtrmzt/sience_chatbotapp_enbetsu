...
export default function AdminPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const data = await apiRequest('/api/worksheets?includeUnpublished=true', 'GET');
        ...
        {isLoading ? <p>データを読み込んでいます...</p> : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-4 rounded-lg shadow">
                    ...
                    <ul className="space-y-2 max-h-[70vh] overflow-y-auto">{lessons.map(lesson => (<li key={lesson.id} onClick={() => { setSelectedLesson(lesson); setSelectedTopic(null); setSelectedProblem(null); }} className={`p-2 rounded cursor-pointer flex justify-between items-center ${selectedLesson?.id === lesson.id ? 'bg-blue-100' : 'hover:bg-gray-100'}`}><span className="text-black">{lesson.name}</span><button onClick={(e) => { e.stopPropagation(); handleDelete('lesson', lesson); }} className="text-red-500 hover:text-red-700 text-xs">削除</button></li>))}</ul>
                </div>
                ...
                <div className="bg-white p-4 rounded-lg shadow">
                    ...
                    {selectedLesson && (<ul className="space-y-2 max-h-[70vh] overflow-y-auto">{selectedLesson.topics.map(topic => (<li key={topic.id} onClick={() => { setSelectedTopic(topic); setSelectedProblem(null); }} className={`p-2 rounded cursor-pointer flex justify-between items-center ${selectedTopic?.id === topic.id ? 'bg-blue-100' : 'hover:bg-gray-100'}`}><span className="text-black">{topic.name}</span><button onClick={(e) => { e.stopPropagation(); handleDelete('topic', topic); }} className="text-red-500 hover:text-red-700 text-xs">削除</button></li>))}</ul>)}
                </div>
                        ...
                        <div className="max-h-[70vh] overflow-y-auto">
                            ...
                            {selectedProblem && (
                                <form onSubmit={handleUpdateProblem} className="space-y-4 border-t pt-6">
                                    <div><label htmlFor="title" className="block text-sm font-medium text-gray-900">設問</label><input type="text" id="title" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black" required/></div>
                                    <div><label htmlFor="question" className="block text-sm font-medium text-gray-900">最初の問いかけ</label><textarea id="question" rows={3} value={formData.question} onChange={e => setFormData({...formData, question: e.target.value})} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black" required/></div>
                                    <div><label htmlFor="prompt" className="block text-sm font-medium text-gray-900">プロンプト内容</label><textarea id="prompt" rows={10} value={formData.prompt} onChange={e => setFormData({...formData, prompt: e.target.value})} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-mono text-black" required/></div>
